...
var RegisterContentScripts = function(pageDomains)
{
	...
	{
		if (pageDomain == "youtube.com") contentScriptFiles = data.url("youtube.com-orchestrator.js");
		else contentScriptFiles = [data.url("Finder.js"), data.url(pageDomain + "-view.js"), data.url("orchestrator.js")];
...
var AttachWorkerToPage = function(worker)
{
	...
	worker.port.on("Broadcast", EmitEventToInactivePageWorkers);
...
var EmitEventToInactivePageWorkers = function(eventData)
{
	for(worker in pageWorkers)
	{
		if (worker != this)
		{
			console.log("emitting " + eventData + " to " + worker);
			pageWorkers[activePageWorkerIndex].port.emit(eventData);
		}
	}
}
