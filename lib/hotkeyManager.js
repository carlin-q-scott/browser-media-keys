...
var RegisterPageMods = function()
{
	...
	for(let pageModSpec of pageModSpecs)
    {
        ...
        if (pageModSpec.include.startsWith("@"))
        {
            ...
            preferences.on(prefName, PreferenceChangeHandler);
        }
        pageModSpec.onAttach = AttachWorkerToPage;

        var mod = pageMod.PageMod(pageModSpec);
        if (typeof prefName !== 'undefined') pageMods[prefName] = mod;
	}
};

var PreferenceChangeHandler = function(prefName)
{
	var tabs = require("sdk/tabs");
    var pageMod = pageMods[prefName];
	var { List } = require("sdk/util/list");

	//parse new preferences
	var newInclude = this.prefs[prefName].split(" ").pop();
	List.addListItem(pageMod.include, newInclude);

	//apply mod to newly included tabs
	var pattern = new MatchPattern(newInclude);
	for(let tab in tabs) if (pattern.test(tab.url)) tab.attach(pageMod);
};
