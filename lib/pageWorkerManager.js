...
function TogglePin(xulTabWithWorker)
{
    var tabId = require("sdk/tabs/utils").getTabId(xulTabWithWorker);
...
function SetUpPinnerForWindow(window){
    ...
    newTabContextMenuItem.setAttribute("label", "Pin/Unpin Media Keys");
    newTabContextMenuItem.setAttribute("accesskey", "k");
    newTabContextMenuItem.setAttribute("oncommand", "MediaKeys.TogglePin(gBrowser.mContextTab)");
    
    let menu = doc.getElementById("tabContextMenu");
    menu.insertBefore(newTabContextMenuItem, menu.firstChild);
    
    /*
    This is an attempt to customize the menu item based on whether it's associated with a MediaKeys tab, and whether that tab is pinned or not.
    Unforunately gBrowser.mContextTab is null in the menu context and I see no way to get the tab for the current context.
    
    menu.setAttribute("onpopupshowing", "MediaKeys.ShowCorrectMenuItem(gBrowser.mContextTab, event.target)");
    doc.MediaKeys.ShowCorrectMenuItem = function(xulTab, menuitem){
