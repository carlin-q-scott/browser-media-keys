...
var AttachWorkerToPage = function(worker)
{
    ...
    worker.on('detach', function(){
        DetachPageWorker(this);
    });
    worker.tab.on('activate', function(){
        ActivatePageWorker(worker);
    });

    worker.on('Play', function(){ EmitEventToLastActivePageWorker("MediaPause"); });
    worker.on('Pause', function(){ EmitEventToLastActivePageWorker("MediaPlay"); });
    worker.on('Stop', function(){ EmitEventToLastActivePageWorker("MediaPlay"); });
};
...
var DetachPageWorker = function(worker)
{
    ...
    setTimeout(function(){
        if (pageWorkers.length == 1)
        {
            EmitEventToActivePageWorker("MediaPlay");
        }
...
var EmitEventToLastActivePageWorker = function(eventData)
{
    var lastActivePageWorkerIndex = pageWorkers.length - 2;
    if (lastActivePageWorkerIndex < 0) return;
    //for(var idx = 0; idx < pageWorkers.length - 1; idx++)
    //{
    var pageWorker = pageWorkers[lastActivePageWorkerIndex];
    console.log(`emitting ${eventData} to ${pageWorker.url}`);
